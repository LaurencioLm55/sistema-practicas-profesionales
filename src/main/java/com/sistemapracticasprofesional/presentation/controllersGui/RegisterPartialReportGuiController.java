package com.sistemapracticasprofesional.presentation.controllersGui;

import com.itextpdf.text.DocumentException;
import com.sistemapracticasprofesional.logic.controllers.InternController;
import com.sistemapracticasprofesional.logic.controllers.PartialReportController;
import com.sistemapracticasprofesional.logic.dto.InternDto;
import com.sistemapracticasprofesional.logic.dto.PartialReportActivityDto;
import com.sistemapracticasprofesional.logic.dto.PartialReportDto;
import com.sistemapracticasprofesional.logic.dto.PartialReportPdfData;
import com.sistemapracticasprofesional.logic.exception.BusinessLogicException;
import com.sistemapracticasprofesional.logic.util.PartialReportPdfGenerator;
import com.sistemapracticasprofesional.presentation.util.Navigation;
import com.sistemapracticasprofesional.presentation.util.UserSession;
import java.io.File;
import java.io.IOException;
import java.net.URL;
import java.util.ArrayList;
import java.util.List;
import java.util.ResourceBundle;
import javafx.collections.FXCollections;
import javafx.collections.ObservableList;
import javafx.event.ActionEvent;
import javafx.fxml.FXML;
import javafx.fxml.Initializable;
import javafx.scene.control.Alert;
import javafx.scene.control.ListView;
import javafx.scene.control.TextArea;
import javafx.scene.control.TextField;
import javafx.stage.FileChooser;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

public class RegisterPartialReportGuiController implements Initializable {

    private static final Logger LOGGER = LoggerFactory.getLogger(RegisterPartialReportGuiController.class);

    @FXML
    private TextField educationalExperienceNameField;

    @FXML
    private TextField courseRegistrationNumberField;

    @FXML
    private TextField projectNameField;

    @FXML
    private TextField organizationNameField;

    @FXML
    private TextField activityNameField;

    @FXML
    private TextArea activityDescriptionArea;

    @FXML
    private TextField plannedTimeField;

    @FXML
    private TextField realTimeField;

    @FXML
    private TextField monthField;

    @FXML
    private TextField weekField;

    @FXML
    private ListView<String> activitiesListView;

    private final ObservableList<String> activitySummaries = FXCollections.observableArrayList();
    private final List<PartialReportActivityDto> activities = new ArrayList<>();

    private final PartialReportController partialReportController = new PartialReportController();
    private final InternController internController = new InternController();

    private InternDto intern;

    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {

        activitiesListView.setItems(activitySummaries);
        loadIntern();

    }

    private void loadIntern() {

        try {

            int userId = UserSession.getInstance().getIdUser();
            intern = internController.getInternByUserId(userId);

        } catch (BusinessLogicException e) {

            LOGGER.error("No se pudo obtener el practicante en sesión", e);
            showAlert(Alert.AlertType.ERROR, "No se pudo cargar la información del practicante.");

        }

    }

    @FXML
    private void handleAddActivity(ActionEvent event) {

        String activityName = activityNameField.getText().trim();
        String activityDescription = activityDescriptionArea.getText().trim();
        String plannedTime = plannedTimeField.getText().trim();
        String realTime = realTimeField.getText().trim();
        String month = monthField.getText().trim();
        String week = weekField.getText().trim();

        if (activityName.isEmpty() || activityDescription.isEmpty() || plannedTime.isEmpty()
                || realTime.isEmpty() || month.isEmpty() || week.isEmpty()) {

            showAlert(Alert.AlertType.WARNING, "Completa todos los campos de la actividad antes de agregarla.");
            return;

        }

        PartialReportActivityDto activity = new PartialReportActivityDto();
        activity.setActivityName(activityName);
        activity.setDescription(activityDescription);
        activity.setPlannedTime(plannedTime);
        activity.setRealTime(realTime);
        activity.setMonth(month);
        activity.setWeek(week);

        activities.add(activity);
        activitySummaries.add(buildActivitySummary(activities.size(), activity));

        activityNameField.clear();
        activityDescriptionArea.clear();
        plannedTimeField.clear();
        realTimeField.clear();
        monthField.clear();
        weekField.clear();

    }

    @FXML
    private void handleRemoveActivity(ActionEvent event) {

        int selectedIndex = activitiesListView.getSelectionModel().getSelectedIndex();

        if (selectedIndex < 0) {

            showAlert(Alert.AlertType.WARNING, "Selecciona una actividad de la lista para eliminarla.");
            return;

        }

        activities.remove(selectedIndex);
        activitySummaries.remove(selectedIndex);
        renumberActivitySummaries();

    }

    @FXML
    private void handleGenerateReport(ActionEvent event) {

        if (intern == null) {

            showAlert(Alert.AlertType.ERROR, "No se pudo identificar al practicante en sesión.");
            return;

        }

        String educationalExperienceName = educationalExperienceNameField.getText().trim();
        String courseRegistrationNumber = courseRegistrationNumberField.getText().trim();
        String projectName = projectNameField.getText().trim();
        String organizationName = organizationNameField.getText().trim();

        if (educationalExperienceName.isEmpty() || courseRegistrationNumber.isEmpty()
                || projectName.isEmpty() || organizationName.isEmpty()) {

            showAlert(Alert.AlertType.WARNING, "Completa los datos de la EE y del proyecto.");
            return;

        }

        if (activities.isEmpty()) {

            showAlert(Alert.AlertType.WARNING, "Agrega al menos una actividad antes de generar el reporte.");
            return;

        }

        FileChooser fileChooser = new FileChooser();
        fileChooser.setTitle("Guardar Reporte Parcial");
        fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("PDF", "*.pdf"));
        fileChooser.setInitialFileName("ReporteParcial_" + intern.getStudentId() + ".pdf");
        File outputFile = fileChooser.showSaveDialog(null);

        if (outputFile == null) {
            return;
        }

        PartialReportPdfData reportData = buildReportData(educationalExperienceName, courseRegistrationNumber, projectName, organizationName);

        try {

            PartialReportPdfGenerator.generate(reportData, outputFile);

        } catch (DocumentException | IOException e) {

            LOGGER.error("Error al generar el PDF del reporte parcial", e);
            showAlert(Alert.AlertType.ERROR, "No se pudo generar el PDF: " + e.getMessage());
            return;

        }

        saveActivitiesToDatabase();

        showAlert(Alert.AlertType.INFORMATION, "Reporte parcial generado y registrado correctamente.");

        try {

            Navigation.changeScene(event, "GuiInternsMenu.fxml", "Menú del practicante");

        } catch (IOException e) {

            LOGGER.error("Ruta no encontrada", e);

        }

    }

    private PartialReportPdfData buildReportData(String educationalExperienceName, String courseRegistrationNumber,
            String projectName, String organizationName) {

        PartialReportPdfData reportData = new PartialReportPdfData();
        reportData.setInternName(intern.getName());
        reportData.setStudentId(intern.getStudentId());
        reportData.setEducationalExperienceName(educationalExperienceName);
        reportData.setCourseRegistrationNumber(courseRegistrationNumber);
        reportData.setProjectName(projectName);
        reportData.setOrganizationName(organizationName);
        reportData.setActivities(activities);

        return reportData;

    }

    private void saveActivitiesToDatabase() {

        int uniqueIdentifierBase = generateTimestampBaseIdentifier();

        for (int activityIndex = 0; activityIndex < activities.size(); activityIndex++) {

            PartialReportActivityDto activity = activities.get(activityIndex);
            PartialReportDto partialReportDto = buildPartialReportDto(activity, uniqueIdentifierBase + activityIndex);

            try {

                partialReportController.registerPartialReport(partialReportDto);

            } catch (BusinessLogicException e) {

                LOGGER.error("Error al guardar actividad en BD: {}", activity.getActivityName(), e);

            }

        }

    }

    private PartialReportDto buildPartialReportDto(PartialReportActivityDto activity, int activityId) {

        PartialReportDto partialReportDto = new PartialReportDto();
        partialReportDto.setProyectActivityId(activityId);
        partialReportDto.setPlannedTime(activity.getPlannedTime());
        partialReportDto.setRealTime(activity.getRealTime());
        partialReportDto.setResults(activity.getDescription());
        partialReportDto.setObservations(activity.getActivityName());

        return partialReportDto;

    }

    private int generateTimestampBaseIdentifier() {

        long currentTimeMillis = System.currentTimeMillis();
        return (int) (currentTimeMillis / 1000);

    }

    private void renumberActivitySummaries() {

        for (int activityIndex = 0; activityIndex < activities.size(); activityIndex++) {

            String updatedSummary = buildActivitySummary(activityIndex + 1, activities.get(activityIndex));
            activitySummaries.set(activityIndex, updatedSummary);

        }

    }

    private String buildActivitySummary(int position, PartialReportActivityDto activity) {

        return position + ". " + activity.getActivityName()
                + " — Mes: " + activity.getMonth()
                + ", Semana: " + activity.getWeek();

    }

    @FXML
    private void handleBack(ActionEvent event) {

        try {

            Navigation.changeScene(event, "GuiInternsMenu.fxml", "Menú del practicante");

        } catch (IOException e) {

            LOGGER.error("Ruta no encontrada", e);
            showAlert(Alert.AlertType.ERROR, "No se pudo volver al menú.");

        }

    }

    private void showAlert(Alert.AlertType type, String message) {

        Alert alert = new Alert(type);
        alert.setTitle(null);
        alert.setHeaderText(message);
        alert.showAndWait();

    }

}
